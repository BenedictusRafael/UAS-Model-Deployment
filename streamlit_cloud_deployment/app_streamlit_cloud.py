...
# LOAD MODEL DARI FILE LOKAL ATAU GITHUB
@st.cache_resource
def load_models():
    try:
        ...
        try:
            # Coba dari path alternatif
            import os
            import urllib.request
            
            # Download dari GitHub jika file tidak ada
            base_url = "https://raw.githubusercontent.com/YOUR_USERNAME/YOUR_REPO/main/"
            
            if not os.path.exists('model_compressed.pkl'):
                urllib.request.urlretrieve(base_url + 'model_compressed.pkl', 'model_compressed.pkl')
            if not os.path.exists('preprocessor.pkl'):
                urllib.request.urlretrieve(base_url + 'preprocessor.pkl', 'preprocessor.pkl')
            if not os.path.exists('encoder.pkl'):
                urllib.request.urlretrieve(base_url + 'encoder.pkl', 'encoder.pkl')
            ...
            st.info("Pastikan file model_compressed.pkl, preprocessor.pkl, dan encoder.pkl ada")
...
if model is None:
    st.error("Model tidak ditemukan. Silakan upload file model.")
    st.stop()
        ...
        if errors:
            ...
            with st.spinner("Memproses data..."):
                try:
                    ...
                    st.info("Pastikan model files (model_compressed.pkl, preprocessor.pkl, encoder.pkl) tersedia")

# FOOTER
st.markdown("---")
st.caption("Credit Score Prediction App - Powered by Streamlit Cloud")
